...
func (e *UnsupportedVersionError) Error() string {
	return fmt.Sprintf("unsupported JFR format version %s: legacy FLR v0 recordings are not yet supported", e.Version)
}

func parseVersion(buf []byte) (Version, error) {
	if len(buf) != 4 {
		return Version{}, fmt.Errorf("invalid version payload size: %d", len(buf))
	}
	return Version{
		Major: binary.BigEndian.Uint16(buf[:2]),
		Minor: binary.BigEndian.Uint16(buf[2:]),
	}, nil
}
...
func (c *Chunk) Parse(r io.Reader, options *ChunkParseOptions) (err error) {
	...
	if err != nil {
		return fmt.Errorf("unable to parse format version: %w", err)
	}
	c.Version = version
	if version.Major == 0 {
		fullReader := io.MultiReader(bytes.NewReader(append(append([]byte{}, magic...), buf...)), r)
		return c.parseLegacyV0(version, fullReader, options)
	}
