...
type BindDatadogProfilerClassRefCache struct {
	Temp   DatadogProfilerClassRefCache
	Fields []BindFieldDatadogProfilerClassRefCache
}

type BindFieldDatadogProfilerClassRefCache struct {
	Field  *def.Field
	uint64 *uint64
}

func NewBindDatadogProfilerClassRefCache(typ *def.Class, typeMap *def.TypeMap) *BindDatadogProfilerClassRefCache {
	res := new(BindDatadogProfilerClassRefCache)
	res.Fields = make([]BindFieldDatadogProfilerClassRefCache, 0, len(typ.Fields))
	for i := 0; i < len(typ.Fields); i++ {
		switch typ.Fields[i].Name {
		case "startTime":
			if typ.Fields[i].Equals(&def.Field{Name: "startTime", Type: typeMap.T_LONG, ConstantPool: false, Array: false}) {
				res.Fields = append(res.Fields, BindFieldDatadogProfilerClassRefCache{Field: &typ.Fields[i], uint64: &res.Temp.StartTime})
			} else {
				res.Fields = append(res.Fields, BindFieldDatadogProfilerClassRefCache{Field: &typ.Fields[i]}) // skip changed field
			}
		case "size":
			if typ.Fields[i].Equals(&def.Field{Name: "size", Type: typeMap.T_LONG, ConstantPool: false, Array: false}) {
				res.Fields = append(res.Fields, BindFieldDatadogProfilerClassRefCache{Field: &typ.Fields[i], uint64: &res.Temp.Size})
			} else {
				res.Fields = append(res.Fields, BindFieldDatadogProfilerClassRefCache{Field: &typ.Fields[i]}) // skip changed field
			}
		default:
			res.Fields = append(res.Fields, BindFieldDatadogProfilerClassRefCache{Field: &typ.Fields[i]}) // skip unknown new field
		}
	}
	return res
}

type DatadogProfilerClassRefCache struct {
	StartTime uint64
	Size      uint64
}

func (this *DatadogProfilerClassRefCache) Parse(data []byte, bind *BindDatadogProfilerClassRefCache, typeMap *def.TypeMap) (pos int, err error) {
	var (
		v64_  uint64
		v32_  uint32
		s_    string
