...
type BindQueueTime struct {
	Temp   QueueTime
	Fields []BindFieldQueueTime
}

type BindFieldQueueTime struct {
	Field     *def.Field
	uint64    *uint64
	ThreadRef *ThreadRef
	ClassRef  *ClassRef
...
func NewBindQueueTime(typ *def.Class, typeMap *def.TypeMap) *BindQueueTime {
	res := new(BindQueueTime)
	res.Fields = make([]BindFieldQueueTime, 0, len(typ.Fields))
	for i := 0; i < len(typ.Fields); i++ {
		switch typ.Fields[i].Name {
		case "startTime":
			if typ.Fields[i].Equals(&def.Field{Name: "startTime", Type: typeMap.T_LONG, ConstantPool: false, Array: false}) {
				res.Fields = append(res.Fields, BindFieldQueueTime{Field: &typ.Fields[i], uint64: &res.Temp.StartTime})
			} else {
				res.Fields = append(res.Fields, BindFieldQueueTime{Field: &typ.Fields[i]}) // skip changed field
			}
		case "duration":
			if typ.Fields[i].Equals(&def.Field{Name: "duration", Type: typeMap.T_LONG, ConstantPool: false, Array: false}) {
				res.Fields = append(res.Fields, BindFieldQueueTime{Field: &typ.Fields[i], uint64: &res.Temp.Duration})
			} else {
				res.Fields = append(res.Fields, BindFieldQueueTime{Field: &typ.Fields[i]}) // skip changed field
			}
		case "eventThread":
			if typ.Fields[i].Equals(&def.Field{Name: "eventThread", Type: typeMap.T_THREAD, ConstantPool: true, Array: false}) {
				res.Fields = append(res.Fields, BindFieldQueueTime{Field: &typ.Fields[i], ThreadRef: &res.Temp.EventThread})
			} else {
				res.Fields = append(res.Fields, BindFieldQueueTime{Field: &typ.Fields[i]}) // skip changed field
			}
		case "origin":
			if typ.Fields[i].Equals(&def.Field{Name: "origin", Type: typeMap.T_THREAD, ConstantPool: true, Array: false}) {
				res.Fields = append(res.Fields, BindFieldQueueTime{Field: &typ.Fields[i], ThreadRef: &res.Temp.Origin})
			} else {
				res.Fields = append(res.Fields, BindFieldQueueTime{Field: &typ.Fields[i]}) // skip changed field
			}
		case "task":
			if typ.Fields[i].Equals(&def.Field{Name: "task", Type: typeMap.T_CLASS, ConstantPool: true, Array: false}) {
				res.Fields = append(res.Fields, BindFieldQueueTime{Field: &typ.Fields[i], ClassRef: &res.Temp.Task})
			} else {
				res.Fields = append(res.Fields, BindFieldQueueTime{Field: &typ.Fields[i]}) // skip changed field
			}
		case "scheduler":
			if typ.Fields[i].Equals(&def.Field{Name: "scheduler", Type: typeMap.T_CLASS, ConstantPool: true, Array: false}) {
				res.Fields = append(res.Fields, BindFieldQueueTime{Field: &typ.Fields[i], ClassRef: &res.Temp.Scheduler})
			} else {
				res.Fields = append(res.Fields, BindFieldQueueTime{Field: &typ.Fields[i]}) // skip changed field
			}
		default:
			res.Fields = append(res.Fields, BindFieldQueueTime{Field: &typ.Fields[i]}) // skip unknown new field
		}
	}
	return res
}
...
func (this *QueueTime) Parse(data []byte, bind *BindQueueTime, typeMap *def.TypeMap) (pos int, err error) {
	var (
		v64_  uint64
		v32_  uint32
		s_    string
