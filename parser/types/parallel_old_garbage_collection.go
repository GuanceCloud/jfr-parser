...
type BindParallelOldGarbageCollection struct {
	Temp   ParallelOldGarbageCollection
	Fields []BindFieldParallelOldGarbageCollection
}

type BindFieldParallelOldGarbageCollection struct {
	Field  *def.Field
	uint64 *uint64
	uint32 *uint32
}

func NewBindParallelOldGarbageCollection(typ *def.Class, typeMap *def.TypeMap) *BindParallelOldGarbageCollection {
	res := new(BindParallelOldGarbageCollection)
	res.Fields = make([]BindFieldParallelOldGarbageCollection, 0, len(typ.Fields))
	for i := 0; i < len(typ.Fields); i++ {
		switch typ.Fields[i].Name {
		case "startTime":
			if typ.Fields[i].Equals(&def.Field{Name: "startTime", Type: typeMap.T_LONG, ConstantPool: false, Array: false}) {
				res.Fields = append(res.Fields, BindFieldParallelOldGarbageCollection{Field: &typ.Fields[i], uint64: &res.Temp.StartTime})
			} else {
				res.Fields = append(res.Fields, BindFieldParallelOldGarbageCollection{Field: &typ.Fields[i]}) // skip changed field
			}
		case "duration":
			if typ.Fields[i].Equals(&def.Field{Name: "duration", Type: typeMap.T_LONG, ConstantPool: false, Array: false}) {
				res.Fields = append(res.Fields, BindFieldParallelOldGarbageCollection{Field: &typ.Fields[i], uint64: &res.Temp.Duration})
			} else {
				res.Fields = append(res.Fields, BindFieldParallelOldGarbageCollection{Field: &typ.Fields[i]}) // skip changed field
			}
		case "gcId":
			if typ.Fields[i].Equals(&def.Field{Name: "gcId", Type: typeMap.T_INT, ConstantPool: false, Array: false}) {
				res.Fields = append(res.Fields, BindFieldParallelOldGarbageCollection{Field: &typ.Fields[i], uint32: &res.Temp.GcId})
			} else {
				res.Fields = append(res.Fields, BindFieldParallelOldGarbageCollection{Field: &typ.Fields[i]}) // skip changed field
			}
		case "densePrefix":
			if typ.Fields[i].Equals(&def.Field{Name: "densePrefix", Type: typeMap.T_LONG, ConstantPool: false, Array: false}) {
				res.Fields = append(res.Fields, BindFieldParallelOldGarbageCollection{Field: &typ.Fields[i], uint64: &res.Temp.DensePrefix})
			} else {
				res.Fields = append(res.Fields, BindFieldParallelOldGarbageCollection{Field: &typ.Fields[i]}) // skip changed field
			}
		default:
			res.Fields = append(res.Fields, BindFieldParallelOldGarbageCollection{Field: &typ.Fields[i]}) // skip unknown new field
		}
	}
	return res
}

type ParallelOldGarbageCollection struct {
	StartTime   uint64
	Duration    uint64
	GcId        uint32
	DensePrefix uint64
...
func (this *ParallelOldGarbageCollection) Parse(data []byte, bind *BindParallelOldGarbageCollection, typeMap *def.TypeMap) (pos int, err error) {
	var (
		v64_  uint64
		v32_  uint32
		s_    string
