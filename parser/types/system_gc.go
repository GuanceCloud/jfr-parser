...
type BindSystemGC struct {
	Temp   SystemGC
	Fields []BindFieldSystemGC
}

type BindFieldSystemGC struct {
	Field         *def.Field
	uint64        *uint64
	ThreadRef     *ThreadRef
...
func NewBindSystemGC(typ *def.Class, typeMap *def.TypeMap) *BindSystemGC {
	res := new(BindSystemGC)
	res.Fields = make([]BindFieldSystemGC, 0, len(typ.Fields))
	for i := 0; i < len(typ.Fields); i++ {
		switch typ.Fields[i].Name {
		case "startTime":
			if typ.Fields[i].Equals(&def.Field{Name: "startTime", Type: typeMap.T_LONG, ConstantPool: false, Array: false}) {
				res.Fields = append(res.Fields, BindFieldSystemGC{Field: &typ.Fields[i], uint64: &res.Temp.StartTime})
			} else {
				res.Fields = append(res.Fields, BindFieldSystemGC{Field: &typ.Fields[i]}) // skip changed field
			}
		case "duration":
			if typ.Fields[i].Equals(&def.Field{Name: "duration", Type: typeMap.T_LONG, ConstantPool: false, Array: false}) {
				res.Fields = append(res.Fields, BindFieldSystemGC{Field: &typ.Fields[i], uint64: &res.Temp.Duration})
			} else {
				res.Fields = append(res.Fields, BindFieldSystemGC{Field: &typ.Fields[i]}) // skip changed field
			}
		case "eventThread":
			if typ.Fields[i].Equals(&def.Field{Name: "eventThread", Type: typeMap.T_THREAD, ConstantPool: true, Array: false}) {
				res.Fields = append(res.Fields, BindFieldSystemGC{Field: &typ.Fields[i], ThreadRef: &res.Temp.EventThread})
			} else {
				res.Fields = append(res.Fields, BindFieldSystemGC{Field: &typ.Fields[i]}) // skip changed field
			}
		case "stackTrace":
			if typ.Fields[i].Equals(&def.Field{Name: "stackTrace", Type: typeMap.T_STACK_TRACE, ConstantPool: true, Array: false}) {
				res.Fields = append(res.Fields, BindFieldSystemGC{Field: &typ.Fields[i], StackTraceRef: &res.Temp.StackTrace})
			} else {
				res.Fields = append(res.Fields, BindFieldSystemGC{Field: &typ.Fields[i]}) // skip changed field
			}
		case "invokedConcurrent":
			if typ.Fields[i].Equals(&def.Field{Name: "invokedConcurrent", Type: typeMap.T_BOOLEAN, ConstantPool: false, Array: false}) {
				res.Fields = append(res.Fields, BindFieldSystemGC{Field: &typ.Fields[i], bool: &res.Temp.InvokedConcurrent})
			} else {
				res.Fields = append(res.Fields, BindFieldSystemGC{Field: &typ.Fields[i]}) // skip changed field
			}
		default:
			res.Fields = append(res.Fields, BindFieldSystemGC{Field: &typ.Fields[i]}) // skip unknown new field
		}
	}
	return res
}

type SystemGC struct {
	StartTime         uint64
	Duration          uint64
	EventThread       ThreadRef
	StackTrace        StackTraceRef
	InvokedConcurrent bool
}

func (this *SystemGC) Parse(data []byte, bind *BindSystemGC, typeMap *def.TypeMap) (pos int, err error) {
	var (
		v64_  uint64
		v32_  uint32
		s_    string
