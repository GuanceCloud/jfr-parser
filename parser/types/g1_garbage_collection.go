...
type BindG1GarbageCollection struct {
	Temp   G1GarbageCollection
	Fields []BindFieldG1GarbageCollection
}

type BindFieldG1GarbageCollection struct {
	Field       *def.Field
	uint64      *uint64
	uint32      *uint32
	G1YCTypeRef *G1YCTypeRef
}

func NewBindG1GarbageCollection(typ *def.Class, typeMap *def.TypeMap) *BindG1GarbageCollection {
	res := new(BindG1GarbageCollection)
	res.Fields = make([]BindFieldG1GarbageCollection, 0, len(typ.Fields))
	for i := 0; i < len(typ.Fields); i++ {
		switch typ.Fields[i].Name {
		case "startTime":
			if typ.Fields[i].Equals(&def.Field{Name: "startTime", Type: typeMap.T_LONG, ConstantPool: false, Array: false}) {
				res.Fields = append(res.Fields, BindFieldG1GarbageCollection{Field: &typ.Fields[i], uint64: &res.Temp.StartTime})
			} else {
				res.Fields = append(res.Fields, BindFieldG1GarbageCollection{Field: &typ.Fields[i]}) // skip changed field
			}
		case "duration":
			if typ.Fields[i].Equals(&def.Field{Name: "duration", Type: typeMap.T_LONG, ConstantPool: false, Array: false}) {
				res.Fields = append(res.Fields, BindFieldG1GarbageCollection{Field: &typ.Fields[i], uint64: &res.Temp.Duration})
			} else {
				res.Fields = append(res.Fields, BindFieldG1GarbageCollection{Field: &typ.Fields[i]}) // skip changed field
			}
		case "gcId":
			if typ.Fields[i].Equals(&def.Field{Name: "gcId", Type: typeMap.T_INT, ConstantPool: false, Array: false}) {
				res.Fields = append(res.Fields, BindFieldG1GarbageCollection{Field: &typ.Fields[i], uint32: &res.Temp.GcId})
			} else {
				res.Fields = append(res.Fields, BindFieldG1GarbageCollection{Field: &typ.Fields[i]}) // skip changed field
			}
		case "type":
			if typ.Fields[i].Equals(&def.Field{Name: "type", Type: typeMap.T_G1YCType, ConstantPool: true, Array: false}) {
				res.Fields = append(res.Fields, BindFieldG1GarbageCollection{Field: &typ.Fields[i], G1YCTypeRef: &res.Temp.Type})
			} else {
				res.Fields = append(res.Fields, BindFieldG1GarbageCollection{Field: &typ.Fields[i]}) // skip changed field
			}
		default:
			res.Fields = append(res.Fields, BindFieldG1GarbageCollection{Field: &typ.Fields[i]}) // skip unknown new field
		}
	}
	return res
}

type G1GarbageCollection struct {
	StartTime uint64
	Duration  uint64
	GcId      uint32
	Type      G1YCTypeRef
...
func (this *G1GarbageCollection) Parse(data []byte, bind *BindG1GarbageCollection, typeMap *def.TypeMap) (pos int, err error) {
	var (
		v64_  uint64
		v32_  uint32
		s_    string
	...
	for bindFieldIndex := 0; bindFieldIndex < len(bind.Fields); bindFieldIndex++ {
		...
		for bindArrayIndex := 0; bindArrayIndex < bindArraySize; bindArrayIndex++ {
			if bind.Fields[bindFieldIndex].Field.ConstantPool {
				...
				case typeMap.T_G1YCType:
					if bind.Fields[bindFieldIndex].G1YCTypeRef != nil {
						*bind.Fields[bindFieldIndex].G1YCTypeRef = G1YCTypeRef(v64_)
					}
				}
			} else {
