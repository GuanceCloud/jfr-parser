...
type BindExceptionSample struct {
	Temp   ExceptionSample
	Fields []BindFieldExceptionSample
}

type BindFieldExceptionSample struct {
	Field         *def.Field
	uint64        *uint64
	ThreadRef     *ThreadRef
...
func NewBindExceptionSample(typ *def.Class, typeMap *def.TypeMap) *BindExceptionSample {
	res := new(BindExceptionSample)
	res.Fields = make([]BindFieldExceptionSample, 0, len(typ.Fields))
	for i := 0; i < len(typ.Fields); i++ {
		switch typ.Fields[i].Name {
		case "startTime":
			if typ.Fields[i].Equals(&def.Field{Name: "startTime", Type: typeMap.T_LONG, ConstantPool: false, Array: false}) {
				res.Fields = append(res.Fields, BindFieldExceptionSample{Field: &typ.Fields[i], uint64: &res.Temp.StartTime})
			} else {
				res.Fields = append(res.Fields, BindFieldExceptionSample{Field: &typ.Fields[i]}) // skip changed field
			}
		case "duration":
			if typ.Fields[i].Equals(&def.Field{Name: "duration", Type: typeMap.T_LONG, ConstantPool: false, Array: false}) {
				res.Fields = append(res.Fields, BindFieldExceptionSample{Field: &typ.Fields[i], uint64: &res.Temp.Duration})
			} else {
				res.Fields = append(res.Fields, BindFieldExceptionSample{Field: &typ.Fields[i]}) // skip changed field
			}
		case "eventThread":
			if typ.Fields[i].Equals(&def.Field{Name: "eventThread", Type: typeMap.T_THREAD, ConstantPool: true, Array: false}) {
				res.Fields = append(res.Fields, BindFieldExceptionSample{Field: &typ.Fields[i], ThreadRef: &res.Temp.EventThread})
			} else {
				res.Fields = append(res.Fields, BindFieldExceptionSample{Field: &typ.Fields[i]}) // skip changed field
			}
		case "stackTrace":
			if typ.Fields[i].Equals(&def.Field{Name: "stackTrace", Type: typeMap.T_STACK_TRACE, ConstantPool: true, Array: false}) {
				res.Fields = append(res.Fields, BindFieldExceptionSample{Field: &typ.Fields[i], StackTraceRef: &res.Temp.StackTrace})
			} else {
				res.Fields = append(res.Fields, BindFieldExceptionSample{Field: &typ.Fields[i]}) // skip changed field
			}
		case "type":
			if typ.Fields[i].Equals(&def.Field{Name: "type", Type: typeMap.T_STRING, ConstantPool: false, Array: false}) {
				res.Fields = append(res.Fields, BindFieldExceptionSample{Field: &typ.Fields[i], string: &res.Temp.Type})
			} else {
				res.Fields = append(res.Fields, BindFieldExceptionSample{Field: &typ.Fields[i]}) // skip changed field
			}
		case "message":
			if typ.Fields[i].Equals(&def.Field{Name: "message", Type: typeMap.T_STRING, ConstantPool: false, Array: false}) {
				res.Fields = append(res.Fields, BindFieldExceptionSample{Field: &typ.Fields[i], string: &res.Temp.Message})
			} else {
				res.Fields = append(res.Fields, BindFieldExceptionSample{Field: &typ.Fields[i]}) // skip changed field
			}
		case "sampled":
			if typ.Fields[i].Equals(&def.Field{Name: "sampled", Type: typeMap.T_BOOLEAN, ConstantPool: false, Array: false}) {
				res.Fields = append(res.Fields, BindFieldExceptionSample{Field: &typ.Fields[i], bool: &res.Temp.Sampled})
			} else {
				res.Fields = append(res.Fields, BindFieldExceptionSample{Field: &typ.Fields[i]}) // skip changed field
			}
		case "firstOccurrence":
			if typ.Fields[i].Equals(&def.Field{Name: "firstOccurrence", Type: typeMap.T_BOOLEAN, ConstantPool: false, Array: false}) {
				res.Fields = append(res.Fields, BindFieldExceptionSample{Field: &typ.Fields[i], bool: &res.Temp.FirstOccurrence})
			} else {
				res.Fields = append(res.Fields, BindFieldExceptionSample{Field: &typ.Fields[i]}) // skip changed field
			}
		default:
			res.Fields = append(res.Fields, BindFieldExceptionSample{Field: &typ.Fields[i]}) // skip unknown new field
		}
	}
	return res
}
...
func (this *ExceptionSample) Parse(data []byte, bind *BindExceptionSample, typeMap *def.TypeMap) (pos int, err error) {
	var (
		v64_  uint64
		v32_  uint32
		s_    string
