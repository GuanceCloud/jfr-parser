...
type BindCounterName struct {
	Temp   CounterName
	Fields []BindFieldCounterName
}

type BindFieldCounterName struct {
	Field  *def.Field
	string *string
}

func NewBindCounterName(typ *def.Class, typeMap *def.TypeMap) *BindCounterName {
	res := new(BindCounterName)
	res.Fields = make([]BindFieldCounterName, 0, len(typ.Fields))
	for i := 0; i < len(typ.Fields); i++ {
		switch typ.Fields[i].Name {
		case "value":
			if typ.Fields[i].Equals(&def.Field{Name: "value", Type: typeMap.T_STRING, ConstantPool: false, Array: false}) {
				res.Fields = append(res.Fields, BindFieldCounterName{Field: &typ.Fields[i], string: &res.Temp.Value})
			} else {
				res.Fields = append(res.Fields, BindFieldCounterName{Field: &typ.Fields[i]}) // skip changed field
			}
		default:
			res.Fields = append(res.Fields, BindFieldCounterName{Field: &typ.Fields[i]}) // skip unknown new field
		}
	}
	return res
}
...
func (this *CounterName) Parse(data []byte, bind *BindCounterName, typeMap *def.TypeMap) (pos int, err error) {
	var (
		v64_  uint64
		v32_  uint32
		s_    string
