...
type BindHeapLiveObject struct {
	Temp   HeapLiveObject
	Fields []BindFieldHeapLiveObject
}

type BindFieldHeapLiveObject struct {
	Field         *def.Field
	uint64        *uint64
	ThreadRef     *ThreadRef
...
func NewBindHeapLiveObject(typ *def.Class, typeMap *def.TypeMap) *BindHeapLiveObject {
	res := new(BindHeapLiveObject)
	res.Fields = make([]BindFieldHeapLiveObject, 0, len(typ.Fields))
	for i := 0; i < len(typ.Fields); i++ {
		switch typ.Fields[i].Name {
		case "startTime":
			if typ.Fields[i].Equals(&def.Field{Name: "startTime", Type: typeMap.T_LONG, ConstantPool: false, Array: false}) {
				res.Fields = append(res.Fields, BindFieldHeapLiveObject{Field: &typ.Fields[i], uint64: &res.Temp.StartTime})
			} else {
				res.Fields = append(res.Fields, BindFieldHeapLiveObject{Field: &typ.Fields[i]}) // skip changed field
			}
		case "eventThread":
			if typ.Fields[i].Equals(&def.Field{Name: "eventThread", Type: typeMap.T_THREAD, ConstantPool: true, Array: false}) {
				res.Fields = append(res.Fields, BindFieldHeapLiveObject{Field: &typ.Fields[i], ThreadRef: &res.Temp.EventThread})
			} else {
				res.Fields = append(res.Fields, BindFieldHeapLiveObject{Field: &typ.Fields[i]}) // skip changed field
			}
		case "stackTrace":
			if typ.Fields[i].Equals(&def.Field{Name: "stackTrace", Type: typeMap.T_STACK_TRACE, ConstantPool: true, Array: false}) {
				res.Fields = append(res.Fields, BindFieldHeapLiveObject{Field: &typ.Fields[i], StackTraceRef: &res.Temp.StackTrace})
			} else {
				res.Fields = append(res.Fields, BindFieldHeapLiveObject{Field: &typ.Fields[i]}) // skip changed field
			}
		case "objectClass":
			if typ.Fields[i].Equals(&def.Field{Name: "objectClass", Type: typeMap.T_CLASS, ConstantPool: true, Array: false}) {
				res.Fields = append(res.Fields, BindFieldHeapLiveObject{Field: &typ.Fields[i], ClassRef: &res.Temp.ObjectClass})
			} else {
				res.Fields = append(res.Fields, BindFieldHeapLiveObject{Field: &typ.Fields[i]}) // skip changed field
			}
		case "age":
			if typ.Fields[i].Equals(&def.Field{Name: "age", Type: typeMap.T_LONG, ConstantPool: false, Array: false}) {
				res.Fields = append(res.Fields, BindFieldHeapLiveObject{Field: &typ.Fields[i], uint64: &res.Temp.Age})
			} else {
				res.Fields = append(res.Fields, BindFieldHeapLiveObject{Field: &typ.Fields[i]}) // skip changed field
			}
		case "size":
			if typ.Fields[i].Equals(&def.Field{Name: "size", Type: typeMap.T_LONG, ConstantPool: false, Array: false}) {
				res.Fields = append(res.Fields, BindFieldHeapLiveObject{Field: &typ.Fields[i], uint64: &res.Temp.Size})
			} else {
				res.Fields = append(res.Fields, BindFieldHeapLiveObject{Field: &typ.Fields[i]}) // skip changed field
			}
		case "weight":
			if typ.Fields[i].Equals(&def.Field{Name: "weight", Type: typeMap.T_FLOAT, ConstantPool: false, Array: false}) {
				res.Fields = append(res.Fields, BindFieldHeapLiveObject{Field: &typ.Fields[i], float32: &res.Temp.Weight})
			} else {
				res.Fields = append(res.Fields, BindFieldHeapLiveObject{Field: &typ.Fields[i]}) // skip changed field
			}
		default:
			res.Fields = append(res.Fields, BindFieldHeapLiveObject{Field: &typ.Fields[i]}) // skip unknown new field
		}
	}
	return res
}

type HeapLiveObject struct {
	StartTime   uint64
...
func (this *HeapLiveObject) Parse(data []byte, bind *BindHeapLiveObject, typeMap *def.TypeMap) (pos int, err error) {
	var (
		v64_  uint64
		v32_  uint32
		s_    string
