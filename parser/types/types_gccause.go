...
type BindGCCause struct {
	Temp   GCCause
	Fields []BindFieldGCCause
}

type BindFieldGCCause struct {
	Field  *def.Field
	string *string
}

func NewBindGCCause(typ *def.Class, typeMap *def.TypeMap) *BindGCCause {
	res := new(BindGCCause)
	res.Fields = make([]BindFieldGCCause, 0, len(typ.Fields))
	for i := 0; i < len(typ.Fields); i++ {
		switch typ.Fields[i].Name {
		case "name":
			if typ.Fields[i].Equals(&def.Field{Name: "name", Type: typeMap.T_STRING, ConstantPool: false, Array: false}) {
				res.Fields = append(res.Fields, BindFieldGCCause{Field: &typ.Fields[i], string: &res.Temp.Name})
			} else {
				res.Fields = append(res.Fields, BindFieldGCCause{Field: &typ.Fields[i]}) // skip changed field
			}
		default:
			res.Fields = append(res.Fields, BindFieldGCCause{Field: &typ.Fields[i]}) // skip unknown new field
		}
	}
	return res
}

type GCCauseRef uint64

type GCCause struct {
	Name string
}

func (this *GCCause) Parse(data []byte, bind *BindGCCause, typeMap *def.TypeMap) (pos int, err error) {
	var (
		v64_  uint64
		v32_  uint32
		s_    string
