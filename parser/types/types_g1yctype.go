...
type BindG1YCType struct {
	Temp   G1YCType
	Fields []BindFieldG1YCType
}

type BindFieldG1YCType struct {
	Field  *def.Field
	string *string
}

func NewBindG1YCType(typ *def.Class, typeMap *def.TypeMap) *BindG1YCType {
	res := new(BindG1YCType)
	res.Fields = make([]BindFieldG1YCType, 0, len(typ.Fields))
	for i := 0; i < len(typ.Fields); i++ {
		switch typ.Fields[i].Name {
		case "type":
			if typ.Fields[i].Equals(&def.Field{Name: "type", Type: typeMap.T_STRING, ConstantPool: false, Array: false}) {
				res.Fields = append(res.Fields, BindFieldG1YCType{Field: &typ.Fields[i], string: &res.Temp.Type})
			} else {
				res.Fields = append(res.Fields, BindFieldG1YCType{Field: &typ.Fields[i]}) // skip changed field
			}
		default:
			res.Fields = append(res.Fields, BindFieldG1YCType{Field: &typ.Fields[i]}) // skip unknown new field
		}
	}
	return res
}

type G1YCTypeRef uint64

type G1YCType struct {
	Type string
}

func (this *G1YCType) Parse(data []byte, bind *BindG1YCType, typeMap *def.TypeMap) (pos int, err error) {
	var (
		v64_  uint64
		v32_  uint32
		s_    string
